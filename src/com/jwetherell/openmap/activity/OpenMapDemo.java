...
public class OpenMapDemo extends CenteredMapActivity {
	...
	private boolean handleClick(int latE6, int lonE6) {
		...
		if ((UserData.getAzimuth() == UserData.AZIMUTH.DEGREES_180)
				|| (UserData.getAzimuth() == UserData.AZIMUTH.DEGREES_360)) {
			units = "degrees";
			azimuth = ProjMath.radToDeg(azimuth);
			if (UserData.getAzimuth() == UserData.AZIMUTH.DEGREES_360)
